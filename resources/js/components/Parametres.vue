...
const handleSubmit = async () => {
  try {
    const response = await axios.post('/api/store-user-data', formData.value);
...
const fetchPhoto = async () => {
  try {
    const response = await axios.post('/api/get-photo');
    formData.value.imgPath = response.data.photoPath;
  } catch (error) {
...
onMounted(async () => {
  try {
    ...
    formData.value.signature = userData.signature;
    formData.value.appKey = userData.appKey;

    // Fetch user photo after getting user data
    await fetchPhoto();
  } catch (error) {
...
const handlePhotoSubmit = async () => {
  ...
  try {
    const response = await axios.post('/api/upload-photo', fileData, {
      headers: {
...
<template>
    ...
    <div class="col-span-5 xl:col-span-3">
      ...
      >
        <div class="border-b border-stroke py-4 px-7 dark:border-strokedark">
          <h3 class="font-medium text-black dark:text-white">Informations personnelles</h3>
        </div>
        <div class="p-7">
  ...
  ></textarea>
            ...
            <div class="flex justify-end gap-4.5">
              ...
              <button
                class="flex justify-center rounded bg-indigo-600 py-2 px-6 bg-blue-600 font-medium text-white hover:bg-opacity-90"
                type="submit"
              >
              Enregistrer
              </button>
    ...
    <div class="col-span-5 xl:col-span-2">
      ...
      >
        <div class="border-b border-stroke py-4 px-7 dark:border-strokedark">
          <h3 class="font-medium text-black dark:text-white">Votre photo</h3>
        </div>
        <div class="p-7">
  ...
  <div class="mb-4 flex items-center gap-3">
    <div class="h-14 w-14 rounded-full">
      <!-- Display user photo here if available -->
  <img v-if="formData.imgPath" :src="formData.imgPath" alt="User Photo" class="h-full w-full rounded-full">
    </div>
    <div>
      <span class="mb-1.5 font-medium text-black dark:text-white">Modifier Votre photo</span>
      <span class="flex gap-2.5">
        <!-- Optionally, you can add buttons to delete or update the photo -->
        <!-- Example:
          <button class="text-sm font-medium hover:text-primary" @click="deletePhoto">Supprimer</button>
          <button class="text-sm font-medium hover:text-primary" @click="updatePhoto">Mettre à jour</button>
        -->
      </span>
    </div>
  </div>
  ...
  <div id="FileUpload" class="relative mb-5.5 block w-full cursor-pointer appearance-none rounded border-2 border-dashed border-primary bg-gray py-4 px-4 dark:bg-meta-4 sm:py-7.5">
    ...
    <div class="flex flex-col items-center justify-center space-y-3">
      ...
      <p class="text-sm font-medium">
        <span class="text-primary">Cliquez pour téléverser</span> ou faites glisser et déposez
      </p>
    </div>
  </div>
  ...
  <div class="flex justify-end gap-4.5">
    <button class="flex justify-center rounded bg-indigo-600 py-2 px-6 font-medium text-white hover:bg-opacity-90" type="submit">
      Enregistrer
    </button>
  </div>
</form>
